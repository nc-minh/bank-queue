    ...
    while retries < max_retries:
        try:
            if is_exiting:
                ...
                break
            response = requests.get(f'{queue_server_url}/dequeue?key=payment-service', timeout=1200000)
    ...
    while True:
        try:
            ...
            if message:
                ...
                conn.execute('UPDATE orders SET status = ? WHERE order_id = ?', ('PREPARING', message_object["order_id"]))
                conn.commit()
                conn.close()
...
def index():
    conn = get_db_connection()
    rows = conn.execute('SELECT * FROM orders WHERE status = ?', ('PREPARING',)).fetchall()
    conn.close()
...
@app.route('/delivering', methods=['POST'])
def delivering():
